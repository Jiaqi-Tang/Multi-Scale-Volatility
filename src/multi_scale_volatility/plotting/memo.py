...
def create_memo_plots(
    ...
    shuffle_decomposition = read_decomposition(
        paths.shuffle_decomposition_csv, k)
    volatility = read_volatility(paths.volatility_csv, k)
    entropy = read_layer_entropy(paths.layer_entropy_csv, k)
...
def plot_memo_return_distribution(
    ...
    low = min(float(gaussian_quantiles.min()),
              float(empirical_quantiles.min()))
    high = max(float(gaussian_quantiles.max()),
               float(empirical_quantiles.max()))
    qq_axis.plot([low, high], [low, high],
...
def plot_memo_abs_return_acf(
    ...
    axis.plot(lags, final_acf, color=FINAL_COLOR,
              linewidth=1.4, label="EUR/USD")
    axis.plot(
        lags,
        shuffle_acf,
...
def plot_memo_cross_scale_correlation(
    ...
    final_corr = absolute_component_correlation(
        final_decomposition, components)
    ...
    final_colorbar = fig.colorbar(
        final_image, ax=final_axis, fraction=0.046, pad=0.04)
