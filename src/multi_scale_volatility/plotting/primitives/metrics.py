...
def plot_series_metric(
    ...
    for series in SERIES_ORDER:
        ...
        if series_frame[metric].isna().any():
            ...
            raise ValueError(
                f"Missing {metric} values for {series}: {missing}")
        ax.plot(
            components,
...
def plot_baseline_difference_metric(
    ...
    wide = frame.pivot(index=COMPONENT, columns=SERIES,
                       values=metric).reindex(components)
    ...
    if wide[required_series].isna().any().any():
        missing_components = wide[wide[required_series].isna().any(
            axis=1)].index.tolist()
        raise ValueError(
            f"Missing {metric} values for components: {missing_components}")
...
def plot_entropy_gaps(frame: pd.DataFrame, output_path: Path, k: int) -> Path:
    ...
    if ordered[gap_columns].isna().any().any():
        missing = ordered[ordered[gap_columns].isna().any(axis=1)
                          ].index.tolist()
        raise ValueError(
            f"Missing entropy gap values for components: {missing}")
