...
def plot_entropy_pattern_distribution_grid(
    ...
    for component in components:
        ...
        if total <= 0:
            raise ValueError(
                f"Pattern counts sum to zero for {series} {component}")
        percentages = np.array(
            [counts[pattern] / total for pattern in patterns])
    ...
    for axis, component in zip(axes_flat, components):
        ...
        axis.bar(patterns, percentages,
                 color=SERIES_COLORS[series], alpha=0.8, width=0.72)
        axis.axhline(
            1.0 / len(patterns),
            color="black",
            linestyle="--",
...
def plot_layer_histogram_grid(
    ...
    for axis, layer in zip(axes_flat, layers):
        ...
        axis.axvline(np.mean(final_values),
                     color=FINAL_DARK_COLOR, linewidth=0.8)
        axis.axvline(
