...
from multi_scale_volatility.entropy import EntropyPaths, compute_entropy_metrics
...
def run_core_pipeline(options: PipelineOptions | None = None) -> dict[str, Any]:
    ...
    stages = [
        ...
        ("decomposition", lambda: run_decomposition(
            DecompositionPaths(), k=options.k)),
        ("volatility", lambda: compute_volatility_metrics(
            VolatilityPaths(), k=options.k)),
...
def run_plot_pipeline(options: PipelineOptions | None = None) -> dict[str, list[Path]]:
    ...
    stages = [
        ("eda_plots", lambda: create_eda_plots(EdaPlotPaths())),
        (
            "decomposition_plots",
            lambda: create_decomposition_plots(
                DecompositionPlotPaths(), k=options.k),
        ),
        (
            "volatility_plots",
            lambda: create_volatility_plots(
                VolatilityPlotPaths(), k=options.k),
        ),
        ("entropy_plots", lambda: create_entropy_plots(
            EntropyPlotPaths(), k=options.k)),
