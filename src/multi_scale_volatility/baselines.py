...
from multi_scale_volatility.config.columns import LOG_RETURN, TIMESTAMP_UTC
from multi_scale_volatility.config.constants import GAUSSIAN_SEED, SHUFFLE_SEED
...
class BaselinePaths:
    ...
    def shuffle_csv(self) -> Path:
        return resolve_artifact_path(self.output_dir, BASELINES_DIR, SHUFFLE_RETURNS_CSV)

    @property
    def gaussian_csv(self) -> Path:
        return resolve_artifact_path(self.output_dir, BASELINES_DIR, GAUSSIAN_RETURNS_CSV)
